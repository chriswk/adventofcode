...
fun IntArray.toMutableMap(): MutableMap<Long, Long> =
    this.withIndex().associate { it.index.toLong() to it.value.toLong() }.toMutableMap()

fun LongArray.toMutableMap(): MutableMap<Long, Long> =
    this.withIndex().associate { it.index.toLong() to it.value }.toMutableMap()
...
internal class LocalDateTimeProgressionIterator(
    start: LocalDateTime,
...
class LocalDateTimeProgression(
    ...
    override fun iterator(): Iterator<LocalDateTime> =
        LocalDateTimeProgressionIterator(start, endInclusive, stepMinutes)
...
fun <T> Collection<T>.pairs(): List<Pair<T, T>> =
    this.flatMapIndexed { index, a -> this.drop(index).map { b -> a to b } }
...
enum class CompassDirection(val delta: Point2D) {
    North(Point2D(0, 1)),
    NorthEast(Point2D(1, 1)),
    East(Point2D(1, 0)),
    SouthEast(Point2D(1, -1)),
    ...
    NorthWest(Point2D(-1, 1));
    companion object {
        fun fromString(s: String): CompassDirection {
            return when(s) {
                "U" -> North
                "R" -> East
                "D" -> South
                "L" -> West
                else -> throw IllegalArgumentException("Don't know how to handle $s")
            }
        }
    }
}
